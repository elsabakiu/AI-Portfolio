...
const MARKETS_NAV: NavItem[] = [
  { to: "/dashboard", icon: LayoutDashboard, label: "Dashboard",     shortLabel: "Home",     end: true },
...
function BottomTabItem({ item }: { item: NavItem }) {
  return (
    <NavLink
      ...
      className={({ isActive }) =>
        cn(
          "flex flex-1 flex-col items-center justify-center gap-0.5 py-2 text-xs font-medium transition-colors min-w-0",
...
export function Sidebar() {
  return (
    <>
      ...
      <aside className="hidden md:flex w-56 shrink-0 border-r border-border bg-card flex-col h-[calc(100vh-3.5rem)] sticky top-14">
        {/* Nav sections */}
