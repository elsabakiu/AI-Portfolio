...
def _acquire_run_slot_or_raise(timeout_s: float = 90.0) -> None:
    ...
    if status == "queue_full":
        stats = run_limiter.stats()
        raise HTTPException(
            status_code=429,
            detail=(
                f"Run queue is full (active={stats['active']}/{stats['max_active']}, "
                f"queued={stats['queued']}/{stats['max_queued']}). Try again shortly."
            ),
        )
    raise HTTPException(status_code=429, detail="Run queue wait timeout exceeded. Try again shortly.")
        ...
        try:
            ...
            if status != "acquired":
                msg = (
                    ...
                    if status == "queue_full"
                    else "Run queue wait timeout exceeded. Try again shortly."
                )
                loop.call_soon_threadsafe(
                    q.put_nowait, json.dumps({"type": "error", "run_id": run_id, "message": msg})
                )
                return
            acquired = True
            graph = build_graph()
