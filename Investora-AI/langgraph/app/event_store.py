...
def load_all_profile_jsons_with_watchlists() -> List[Dict[str, Any]]:
    """Return all profile JSON dicts with pre-joined watchlists (avoids N+1 reads)."""
    with _conn() as con:
        rows = con.execute(
            """
            SELECT p.user_id AS user_id,
                   p.profile_json AS profile_json,
                   COALESCE(w.tickers, '[]') AS watchlist_json
            FROM user_profiles p
            LEFT JOIN user_watchlists w ON w.user_id = p.user_id
            """
        ).fetchall()
